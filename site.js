...
(function(){
  ...
  function render(){
    ...
    document.getElementById('calc-note').textContent= total===0 ? 'Move the slider to see what a receipt pays.' : capped
      ? 'This receipt would earn '+fmt(raw)+' at '+brand.rate+'%, but the cap is $'+(holder?SB.capHolder:SB.capBase)+' a receipt. '+(holder?'That is the holder cap.':'Hold $STKB and the cap moves to $'+SB.capHolder+'.')
